...
class CSharpLanguage : public ScriptLanguage {
	...
	// W5 SG PoC (v3 pre-research): compile-time [GlobalClass] registry pushed
	// from C# module initializers generated by GodotSharp.SourceGenerators.
	// refresh_global_classes() triggers the <Module> .cctor (via
	// mono_runtime_class_init — plain runtime API, no mono_runtime_invoke, so
	// WASM-safe per eval_2026-07-26_v3_sourcegenerators.md §5.3 方式 A), which
	// calls ScriptRegistry.RegisterGlobalClass → icall → sg_register_global_class
	// below. When sg_registry_populated, the TypeDef reflection scan is skipped.
	HashMap<String, GlobalClassInfo> sg_global_class_cache;
	bool sg_registry_populated = false;
...
public:
	...
	// W5 SG PoC: entry point for the icall pushed from C# module initializers
	// (see sg_global_class_cache above). Main thread only (called from
	// mono_runtime_class_init during refresh_global_classes, or from
	// ScriptRegistry at assembly activation).
	void sg_register_global_class(const String &p_class_name, const String &p_base_type, bool p_is_tool, bool p_is_abstract, const String &p_icon_path);
