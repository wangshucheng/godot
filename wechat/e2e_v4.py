...
# -*- coding: utf-8 -*-
"""
完整 E2E 自动化测试（v4 - 终极方案）

策略：
1. 杀掉所有 wechatdevtools 进程
2. 清空 WeappLog
3. 启动 HTTP CDN 服务器
4. 用 cli.bat open --project --port <http_port> 打开项目
5. 用 cli.bat auto --project --auto-port <ws_port> --trust-project 启用 automator WS
6. 等待 WS 端口可连接
7. 启动 automator_connect.js 连接 ws://127.0.0.1:<ws_port>
   - 监听 consoleLog
   - 触发 reLaunch
...
AUTOMATOR_CONNECT = os.path.join(WS_ROOT, "godot4_7_mono", "wechat", "automator_connect.js")

# 用两个独立端口：HTTP CLI 端口 + WS automator 端口
HTTP_PORT = 9911
WS_PORT = 9422
CDN_PORT = 8000
...
def run_automator(ws_endpoint, timeout_sec=300):
    cmd = ["node", AUTOMATOR_CONNECT, ws_endpoint, str(timeout_sec)]
    log(f"Running automator: {' '.join(cmd)}")
    try:
        r = subprocess.run(
            cmd, capture_output=True, text=True,
            timeout=timeout_sec + 60, encoding="utf-8", errors="ignore",
            cwd=os.path.dirname(AUTOMATOR_CONNECT),
        )
    except subprocess.TimeoutExpired:
...
def main():
    print("=" * 70)
    print("WeChat MiniGame E2E Test (v4: cli open + cli auto + automator.connect)")
    print("=" * 70)
    print(f"Project: {PROJECT_DIR}")
    print(f"HTTP CLI port: {HTTP_PORT}, WS auto port: {WS_PORT}, CDN port: {CDN_PORT}")
    print()
    ...
    log(f"=== Step 4: Open project with --port {HTTP_PORT} ===")
    r1 = cli_cmd(["open", "--project", PROJECT_DIR, "--port", str(HTTP_PORT)], timeout=120)
    if r1 is None or r1.returncode != 0:
        log("open failed", "ERROR")
        return 1

    log(f"=== Step 5: Enable automation with --auto-port {WS_PORT} ===")
    # 关键：用 --auto-port 而不是 --port
    r2 = cli_cmd(
        ["auto", "--project", PROJECT_DIR, "--port", str(HTTP_PORT),
         "--auto-port", str(WS_PORT), "--trust-project"],
        timeout=60,
    )
    # 给 IDE 一些时间真正启动 WS endpoint
    time.sleep(5)

    log(f"=== Step 6: Wait for WS port {WS_PORT} ===")
    if not wait_port("127.0.0.1", WS_PORT, timeout=60):
        log(f"WS port {WS_PORT} not responding", "ERROR")
        # 但仍然尝试连接
    else:
        log(f"WS port {WS_PORT} is listening")
    ...
    log(f"=== Step 8: Run automator (connect {ws_endpoint}) ===")
    auto_results = run_automator(ws_endpoint, timeout_sec=300)
