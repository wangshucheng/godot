// automator_connect.js
// 仅负责连接 + 监听 consoleLog + reLaunch + 等待事件
// 接受 wsEndpoint 作为参数
const automator = require('miniprogram-automator');

const WS_ENDPOINT = process.argv[2] || 'ws://127.0.0.1:9420';
...
const results = {
  boot_start: false, wasm_ok: false, main_ready: false,
  game_ok: false, game_fail: false, compile_err: false,
  canvas_w: null, canvas_h: null,
  diag_canvas_w: null, diag_canvas_h: null,
};
const keyLines = [];
...
async function main() {
  ...
  try {
    ...
    process.stdout.write(JSON.stringify({ error: `connect failed: ${e.message}`, ...results, keyLines }));
    process.exit(3);
  }
