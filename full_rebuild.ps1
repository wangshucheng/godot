...
Write-Host "=== Step 1: scons incremental relink ==="
$startTime = Get-Date
& powershell -NoProfile -ExecutionPolicy Bypass -File "..\build_wasm_interp.ps1" 2>&1 | Select-Object -Last 10
if ($LASTEXITCODE -ne 0) { Write-Host "[ERROR] scons failed"; exit 1 }
$elapsed = (Get-Date) - $startTime
Write-Host "[OK] scons done in $([math]::Round($elapsed.TotalSeconds,0))s"
...
if ($cfgNew -ne $cfg) {
    ...
    Write-Host "[WARN] config.json not updated (pattern not found)"
}
